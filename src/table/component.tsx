...
export function TableHeaderCell({children, ...rest }: TableHeaderCellProps, ref: Ref<HTMLTableCellElement>) {
    const isInTHead = useContext(CellIsInHeaderContext);

    rest.scope ??= (isInTHead? "col" : "row");

    return <th {...useTableHeaderCellProps(rest)} ref={ref}>{children}</th>
}
